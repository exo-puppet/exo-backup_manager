...
# [+weekday+]
#
#   The weekday on which to run the command. Optional; if specified, must be between 0 and 7, inclusive, with 0 (or 7) being Sunday,
#   or must be the name of the day (e.g., Tuesday).
#
# [+special+]
#
#   A special value such as �reboot� or �annually�. Only available on supported systems such as Vixie Cron. Overrides more specific
#   time of day/week settings.
#
# [+source+]
#
#   Source file of the backup-manager configuration file. Valid values are either fully qualified paths to files, or URIs. Currently
...
define backup_manager::define (
  ...
  $special  = undef,
  $source) {
  file { "/etc/backup-manager/${name}.conf":
    ensure  => file,
    owner   => root,
    group   => root,
